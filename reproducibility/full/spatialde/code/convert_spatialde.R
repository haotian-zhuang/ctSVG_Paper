for (f in list.files("svg/crop/full/spatialde/raw_res/", pattern = ".csv.gz")) {
  print(f)
  
  cc <- readRDS(paste0("svg/crop/data/cellclu/", sub(".csv.gz", ".rds", f)))
  raw_res <- read.csv(paste0("svg/crop/full/spatialde/raw_res/", f), row.names = 1, check.names = FALSE)
  ...
  saveRDS(res_spatialde, file = paste0("svg/crop/full/spatialde/", sub(".csv.gz", ".rds", f)))
}
