...
for (f in list.files("/hpc/group/jilab/zj/visiumHD/analysis/visiumHD/visa/filterexpr/", pattern = ".rds")) {
  ...
  fwrite(counts, paste0("svg/crop/down/spatialde/data/counts/", sub(".rds", ".csv.gz", f)), compress = "gzip", row.names = TRUE)
  fwrite(sample_info, paste0("svg/crop/down/spatialde/data/sample_info/", sub(".rds", ".csv.gz", f)), compress = "gzip", row.names = TRUE)
}
