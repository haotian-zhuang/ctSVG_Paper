...
for (f in "mouse_kidney") {
  for (signal in c("v1")) {
    ...
    gene_fail <- c("Creb1")
    spatialRNA <- SpatialRNA(coords = as.data.frame(coord), counts = counts[!rownames(counts) %in% gene_fail, ])
    ...
    runtime = system.time({
      res_cside <- run.CSIDE.nonparam(myRCTD = myRCTD, cell_type_threshold = -Inf, gene_threshold = -Inf,
                                      doublet_mode = FALSE, fdr = Inf, cell_types = setdiff(levels(cc), c("5")))
    })
    gc2 <- gc()
